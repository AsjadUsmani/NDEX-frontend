...
export default function HomePage() {
  return (
    <div style={{ background: '#05080f', minHeight: '100vh', color: '#e8f4f3', fontFamily: "'Inter', 'Space Grotesk', sans-serif", overflowX: 'hidden' }}>

      {/* ── Nav ── */}
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, padding: '0 48px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(5,8,15,0.85)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(0,161,155,0.1)' }}>
        ...
        <button onClick={signInWithGitHub} style={{ height: 36, padding: '0 18px', borderRadius: 8, background: '#00a19b', border: 'none', color: '#05080f', fontWeight: 700, fontSize: 13, cursor: 'pointer', transition: 'filter 0.2s' }} onMouseOver={e => e.currentTarget.style.filter = 'brightness(1.15)'} onMouseOut={e => e.currentTarget.style.filter = 'brightness(1)'}>
          Get Started
        </button>
      </nav>
      ...
      <section style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '120px 24px 80px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 760, textAlign: 'center' }}>
          <motion.div {...fadeUp(0)} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, border: '1px solid rgba(228,221,61,0.3)', background: 'rgba(228,221,61,0.07)', color: '#e4dd3d', fontFamily: "'JetBrains Mono',monospace", fontSize: 12, padding: '5px 14px', borderRadius: 99, marginBottom: 28, letterSpacing: 1 }}>
          ...
          <motion.div {...fadeUp(0.22)} style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 20 }}>
            <button onClick={signInWithGitHub} style={{ display: 'flex', alignItems: 'center', gap: 9, height: 52, padding: '0 28px', background: '#00a19b', color: '#05080f', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: 15, cursor: 'pointer', transition: 'all 0.2s', boxShadow: '0 0 32px rgba(0,161,155,0.35)' }} onMouseOver={e => { e.currentTarget.style.filter = 'brightness(1.12)'; e.currentTarget.style.transform = 'translateY(-2px)' }} onMouseOut={e => { e.currentTarget.style.filter = 'brightness(1)'; e.currentTarget.style.transform = 'translateY(0)' }}>
              <Github size={18} /> Continue with GitHub
            </button>
            <button onClick={signInWithGoogle} style={{ display: 'flex', alignItems: 'center', gap: 9, height: 52, padding: '0 28px', background: 'transparent', color: '#e4dd3d', border: '1px solid rgba(228,221,61,0.3)', borderRadius: 10, fontWeight: 600, fontSize: 15, cursor: 'pointer', transition: 'all 0.2s' }} onMouseOver={e => { e.currentTarget.style.background = 'rgba(228,221,61,0.07)'; e.currentTarget.style.transform = 'translateY(-2px)' }} onMouseOut={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.transform = 'translateY(0)' }}>
          ...
          {/* Mock UI preview */}
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.4, ease: [0.22, 1, 0.36, 1] }} style={{ marginTop: 60, borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(0,161,155,0.18)', background: 'linear-gradient(180deg,rgba(0,161,155,0.04) 0%,transparent)', boxShadow: '0 32px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(0,161,155,0.1) inset' }}>
            <div style={{ padding: '10px 14px', borderBottom: '1px solid rgba(0,161,155,0.1)', display: 'flex', gap: 6, background: 'rgba(8,13,24,0.8)' }}>
            ...
            <div style={{ display: 'flex', height: 340, background: 'rgba(8,13,24,0.6)' }}>
              <div style={{ width: 180, borderRight: '1px solid rgba(0,161,155,0.08)', padding: 14, display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <div style={{ flex: 1, padding: 20, display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '1fr 1fr', gap: 12 }}>
                {[0.08, 0.06, 0.1, 0.05].map((op, i) => (
                  <div key={i} style={{ borderRadius: 10, background: `rgba(0,161,155,${op})`, border: '1px solid rgba(0,161,155,0.12)', position: 'relative', overflow: 'hidden' }}>
                    <div style={{ position: 'absolute', top: 10, left: 12, height: 8, width: '40%', borderRadius: 4, background: 'rgba(0,161,155,0.2)' }} />
                    <div style={{ position: 'absolute', bottom: 10, left: 12, right: 12, height: 50, borderRadius: 6, background: 'rgba(228,221,61,0.06)', border: '1px solid rgba(228,221,61,0.1)' }} />
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 20 }}>
          {features.map((f, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45, delay: i * 0.07 }}
              style={{ background: 'rgba(12,20,32,0.8)', border: '1px solid rgba(0,161,155,0.1)', borderRadius: 14, padding: '28px 24px', cursor: 'default', transition: 'all 0.25s' }}
            ...
            >
              <div style={{ width: 44, height: 44, borderRadius: 12, background: `${f.color}18`, border: `1px solid ${f.color}33`, display: 'grid', placeItems: 'center', marginBottom: 18 }}>
      ...
      {/* ── Stats Bar ── */}
      <section style={{ background: 'rgba(12,20,32,0.6)', borderTop: '1px solid rgba(0,161,155,0.1)', borderBottom: '1px solid rgba(0,161,155,0.1)', padding: '48px' }}>
          ...
          {steps.map((step, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', flex: '1 1 260px', maxWidth: 340 }}>
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45, delay: i * 0.1 }}
                style={{ flex: 1, position: 'relative', background: 'rgba(12,20,32,0.8)', border: '1px solid rgba(0,161,155,0.12)', borderRadius: 16, padding: '32px 28px', overflow: 'hidden' }}>
                ...
                <div style={{ width: 40, height: 40, borderRadius: 12, background: 'rgba(0,161,155,0.15)', border: '1px solid rgba(0,161,155,0.3)', display: 'grid', placeItems: 'center', marginBottom: 18 }}>
      ...
      <section style={{ padding: '80px 48px' }}>
        <motion.div initial={{ opacity: 0, scale: 0.97 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.5 }}
          style={{ maxWidth: 600, margin: '0 auto', background: 'linear-gradient(135deg,rgba(0,161,155,0.1) 0%,rgba(228,221,61,0.05) 100%)', border: '1px solid rgba(228,221,61,0.2)', borderRadius: 20, padding: '60px 48px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <button onClick={signInWithGitHub} style={{ display: 'inline-flex', alignItems: 'center', gap: 10, height: 52, padding: '0 32px', background: '#00a19b', color: '#05080f', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: 15, cursor: 'pointer', transition: 'all 0.2s', boxShadow: '0 0 40px rgba(0,161,155,0.3)' }} onMouseOver={e => { e.currentTarget.style.filter = 'brightness(1.12)'; e.currentTarget.style.transform = 'translateY(-2px)' }} onMouseOut={e => { e.currentTarget.style.filter = 'brightness(1)'; e.currentTarget.style.transform = 'translateY(0)' }}>
      ...
      {/* ── Footer ── */}
      <footer style={{ padding: '40px 48px', borderTop: '1px solid rgba(0,161,155,0.08)' }}>
